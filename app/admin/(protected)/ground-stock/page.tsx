...
import { groundStockSync, productVariants, products } from "@/db/schema";
import {
  PageHeader,
  Card,
  CardHeader,
...
export default async function GroundStockPage({
  ...
  const link = (patch: Record<string, string | number | undefined>) => {
    ...
    const merged = { q, show, page, ...patch };
    ...
    <div>
      ...
      <Card padded={false}>
        <div className="px-5 lg:px-6 pt-5 lg:pt-6 pb-3 flex flex-wrap items-end justify-between gap-3">
          <CardHeader
            title="Per size"
            description={`${count.toLocaleString("en-IN")} row${count === 1 ? "" : "s"}${q ? ` matching “${q}”` : ""}`}
          />
