...
### 3.1 QR codes → production Inventre portal (PUBLIC, no login)
- QR codes are generated by **Audit** and currently encode the
  **dev** URL `http://217.216.58.218:3081/portal`.
- Change them to encode the **plain public URL**
  **`https://inventre.in/portal`** — no token, no login. The portal is now
  a public intake: the parent picks a category, types their name + mobile +
  issue, and gets a `CON-` ticket. (No PII is shown on the page, so it's
  safe to be public.) Tracking is by ticket number at `/portal/history`.

> The concern Admin Panel (view/filter/assign/status/reply/history) is now
> built **natively in the Inventre admin** (`/admin/parent-concerns`), so
> items 3.2–3.4 (pointing the audit admin panel to prod, the audit menu
> split) are **no longer needed** — the support team works concerns in
> Inventre. The only remaining audit-side task is the QR URL (3.1) and,
> optionally, ingesting `concern.created` (3.4) if concerns should also
> appear in audit.
