...
### 3.1 QR codes → production Inventre portal
- QR codes are generated by **Audit** and currently encode the
  **dev** URL `http://217.216.58.218:3081/portal`.
- Change the QR-generation base URL to **`https://inventre.in/portal`**
  (production, https, domain — not an IP, not `:3081`). Update wherever
  the portal base URL is configured (env/setting), so ALL newly generated
  QR codes point to production. (Already-printed labels can't change —
  this is for new generation only.)
