...
const ACTOR = "admin@inventre.in (bulk-script march-paid)";
       ...
       AND EXISTS (
         SELECT 1 FROM mcb_fee_payments p
          WHERE p.enrolment_number = s.enrolment_number AND p.fee_head = 'Tuition fee'
            ...
            AND p.payment_date = (SELECT MAX(p2.payment_date) FROM mcb_fee_payments p2 WHERE p2.enrolment_number = s.enrolment_number AND p2.fee_head = 'Tuition fee')
       )
