...
async function main() {
  ...
  const client = src_default(url, { max: 1, prepare: false });
  try {
    await client`
    ...
    `;
    {
      ...
      const trackedRows = await client`
      ...
      for (const [file, tables] of Object.entries(BOOTSTRAP_FINGERPRINTS)) {
        if (tracked.has(file))
          continue;
        const allPresent = (await Promise.all(tables.map(tableExists))).every(Boolean);
        if (allPresent) {
          await client`
    ...
    const dir = import_path.default.resolve(process.cwd(), "db", "migrations");
    const allFiles = import_fs2.default.readdirSync(dir).filter((f) => f.endsWith(".sql")).sort();
    ...
    for (const file of pending) {
      const sql = import_fs2.default.readFileSync(import_path.default.join(dir, file), "utf8");
      try {
