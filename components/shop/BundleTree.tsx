...
/**
 * Recursive accordion for a bundle's BOM subtree on the PDP.
 *
 * Bundle rows (children.length > 0) expand to reveal the nested tree.
 * Leaf rows expand to reveal that item's variations — picking a variation
 * shows its price and lets the parent add just that variation to the cart.
 */
export function BundleTree({
  ...
  return (
    <ul
      className={
        depth === 0
          ? "divide-y divide-ink-100 rounded-2xl border border-ink-100 overflow-hidden bg-white"
          : "divide-y divide-ink-100 bg-cream-50/40"
      }
    >
      {nodes.map((node) => (
...
function BundleRow({ node, depth, readOnly }: { node: BundleNode; depth: number; readOnly?: boolean }) {
  ...
  if (readOnly && !hasChildren) {
    return (
      <li
        className="flex items-center gap-3 py-2 pr-4 border-b border-ink-50 last:border-0"
      ...
      >
        ...
        {node.qty > 1 && (
          <span className="text-[11px] text-ink-400 tabular-nums shrink-0">× {node.qty}</span>
        )}
      </li>
    );
  }
    ...
    <li>
      ...
      >
        ...
        <span className="flex-1 min-w-0">
          <span className="text-[14px] font-medium text-ink-900 truncate block">
            {node.name}
          </span>
          <span className="text-[12px] text-ink-500">
            {hasChildren ? (
              <>
                ...
                {node.children.length === 1 ? "" : "s"} inside
              </>
            ) : (
      ...
      <AnimatePresence initial={false}>
        {open && (
          ...
          >
            {hasChildren ? (
              <BundleTree nodes={node.children} depth={depth + 1} readOnly={readOnly} />
            ) : (
              <LeafBuyPanel node={node} indent={16 + indent} />
            )}
          </motion.div>
        )}
      </AnimatePresence>
...
function LeafBuyPanel({ node, indent }: { node: BundleNode; indent: number }) {
  ...
  return (
    <div
      className="py-3 pr-4 bg-cream-50/60"
    ...
    >
      ...
      {loaded && variants.length === 0 && (
        <p className="text-[12px] text-ink-500">
          No variations to choose for this item.
        </p>
      )}
          ...
          <div className="mt-3 flex items-center gap-3">
            {picked && picked.pricePaise != null && (
              <span className="text-[15px] font-bold text-ink-900 tabular-nums">
                ...
                {picked.mrpPaise != null &&
                  picked.mrpPaise > picked.pricePaise && (
                    <span className="ml-2 text-[12px] font-normal text-ink-400 line-through">
                      {rupees(picked.mrpPaise)}
                    </span>
                  )}
              </span>
            )}
      ...
      {err && (
        <p className="mt-2 text-[12px] font-medium text-red-500">{err}</p>
      )}
    </div>
  );
}
